...
async function capture(route){try{const html=route==='/'?await fs.readFile('/tmp/oak-reference.html','utf8'):await (await get(origin+route)).text();pages.set(route,html);await fs.writeFile('reference/'+encodeURIComponent(route)+'.html',html);return links(html);}catch(e){errors.push({route,error:String(e)});return [];}}
...
console.log('Primary pages',pages.size,'Product/article pages',secondary.length);await pool(secondary,capture,5);
const catalog=[];
...
let done=new Set();while(done.size<assets.size){const batch=[...assets.entries()].filter(([u])=>!done.has(u));await pool(batch,async([url,file])=>{done.add(url);try{let body=await (await get(url)).arrayBuffer();if(/\.css(?:\?|$)/.test(url))body=css(Buffer.from(body).toString(),url);await fs.writeFile(output+file,typeof body==='string'?body:Buffer.from(body));}catch(e){errors.push({url,error:String(e)});}},10);console.log('Downloaded',done.size,'of',assets.size);}
