...
(() => {
  ...
  const money = cents => `Rs. ${(cents / 100).toLocaleString('en-IN', {
  ...
  const categoryLabels = [
    { value: '', label: 'Kitchen Combos', helper: 'Choose your bundle' },
    { value: 'table-linens', label: 'Table Combos', helper: 'Table linens and dining sets' },
    { value: 'bed-linens', label: 'Bed Combos', helper: 'Sheets, cushions, and bedroom layers' },
    { value: 'living-space', label: 'Living Room Combos', helper: 'Throws, cushions, and living accents' }
  ];
  const packOptions = [
    { label: '3 Sets of Napkins', minimum: 2 },
    { label: 'Any 3 Products', minimum: 3 },
    { label: 'Any 4 Products', minimum: 4 }
  ];
  let selectedPack = packOptions[0];

  function setupBundleUi() {
    const toolbar = root.querySelector('.oak-bundle-toolbar');
    const grid = root.querySelector('.oak-bundle-grid');
    const heading = toolbar?.querySelector('h2');
    const helper = toolbar?.querySelector('p');
    const filter = root.querySelector('[data-filter]');
    if (!toolbar || !grid || !heading || !helper || !filter) return;

    root.querySelector('.oak-bundle-heading h1').textContent = 'Build Your Own Bundle!';
    heading.textContent = categoryLabels[0].label;
    helper.textContent = categoryLabels[0].helper;

    const priority = [
      'oak-lily-lemon-yellow-cotton-cushion-cover-100-pure-cotton-vibrant-washable-european-quality',
      'autumn-leaves-bedsheet',
      'peach-pink-kitchen-napkins',
      'oak-lily-table-runner-in-yellow-green-lemon-print'
    ];
    cards.sort((a, b) => {
      const aIndex = priority.indexOf(a.dataset.product);
      const bIndex = priority.indexOf(b.dataset.product);
      if (aIndex !== -1 || bIndex !== -1) return (aIndex === -1 ? 99 : aIndex) - (bIndex === -1 ? 99 : bIndex);
      return products.get(a.dataset.product).title.localeCompare(products.get(b.dataset.product).title);
    }).forEach(card => grid.append(card));

    const tabs = document.createElement('div');
    tabs.className = 'oak-bundle-pack-tabs';
    tabs.setAttribute('aria-label', 'Choose bundle type');
    tabs.innerHTML = packOptions.map((option, index) =>
      `<button type="button" data-pack-index="${index}" aria-pressed="${index === 0}">${escape(option.label)}</button>`
    ).join('');
    toolbar.append(tabs);

    const nav = document.createElement('div');
    nav.className = 'oak-bundle-category-list';
    nav.innerHTML = categoryLabels.slice(1).map(item =>
      `<button type="button" class="oak-bundle-category-title" data-category-filter="${escape(item.value)}" aria-expanded="false">${escape(item.label)}</button>`
    ).join('');
    grid.after(nav);

    const applyCategory = value => {
      const category = categoryLabels.find(item => item.value === value) || categoryLabels[0];
      filter.value = value;
      heading.textContent = category.label;
      helper.textContent = category.helper;
      cards.forEach(card => {
        card.hidden = Boolean(value) && !card.dataset.categories.split(',').includes(value);
      });
      root.querySelectorAll('[data-category-filter]').forEach(button => {
        button.setAttribute('aria-expanded', String(button.dataset.categoryFilter === value));
      });
    };
    ...
    tabs.addEventListener('click', event => {
      ...
      if (!button) return;
      selectedPack = packOptions[Number(button.dataset.packIndex)] || packOptions[0];
    ...
    nav.addEventListener('click', event => {
      ...
      if (!button) return;
      applyCategory(button.dataset.categoryFilter);
      heading.scrollIntoView({ behavior: 'smooth', block: 'start' });
    });

    heading.addEventListener('click', () => applyCategory(''));
    filter.addEventListener('change', () => applyCategory(filter.value));
    applyCategory('');
  }
  ...
  function render() {
    ...
    const neededForPack = Math.max(selectedPack.minimum - units, 0);
    root.querySelector('[data-tier]').textContent = units >= selectedPack.minimum ? `${selectedPack.label} selected` :
      neededForPack === 1 ? 'Add 1 more product to complete your bundle' :
      `Add ${neededForPack} products to complete your bundle`;
    ...
    root.querySelector('[data-mobile-tier]').textContent = rate ? `${rate}% OFF · ${money(subtotal - discount)}` :
      `Add ${Math.max(2 - units, 0)} ${units === 1 ? 'item' : 'items'} to save 10%`;
    ...
    cards.forEach(card => {
      ...
      button.textContent = !variant?.available ? 'UNAVAILABLE' :
        selected ? 'ADD ANOTHER' : 'ADD TO BAG';
  ...
  root.addEventListener('click', event => {
    ...
    if (busy || !button || button === checkout || button.hasAttribute('data-pack-index') || button.hasAttribute('data-category-filter')) return;
    status.textContent = '';
